...
class ImageController extends AbstractController
{
    ...
    public function postImagesAction(Image $image)
    {
        ...
        return $this->view($image, Response::HTTP_CREATED)->setHeaders([
            'Location' => $this->generateUrl('images_upload_put', ['image' => $image->getId()])
        ]);
    }
    ...
    private function persistImage(?Image $image): void
    {
        $em = $this->getDoctrine()->getManager();
        $em->persist($image);
        $em->flush();
